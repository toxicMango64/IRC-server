...
void Server::Invite( std::string &cmd, int &fd )
{
    ...
    if(scmd[2][0] != '#' || GetChannel(channelname) == nullptr) {
        senderror(403, channelname, fd, " :No such channel\r\n");
        return;
    }

    if (GetChannel(channelname)->get_client(fd) == nullptr && GetChannel(channelname)->get_admin(fd) == nullptr) {
        senderror(442, channelname, fd, " :You're not on that channel\r\n");
        return;
    }

    if (GetChannel(channelname)->GetClientInChannel(scmd[1]) != nullptr) {
    ...
    if (GetChannel(channelname)->GetInvitOnly() != 0 && GetChannel(channelname)->get_admin(fd) == nullptr) {
